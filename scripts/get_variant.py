#!/usr/bin/env python3
"""
Deterministic Variant Computation - Lab 6: Integration Project
...
def generate_parameters(rng, group_id):
    ...
    # Select a subset of drillholes from the CSV (DH-01 through DH-08)
    all_drillholes = [f'DH-{i:02d}' for i in range(1, 9)]
    num_holes = rng.randint(4, 6)
    drillholes = sorted(rng.sample(all_drillholes, k=num_holes))

    # Select analysis columns (always includes grade + 2 others)
    other_columns = ['mass', 'volume', 'from_depth']
    selected_others = rng.sample(other_columns, k=2)
    analysis_columns = sorted(['grade'] + selected_others)

    return {
        'project_name': f"{rng.choice(prefixes)} {rng.choice(suffixes)} Project",
        'target_grade': round(rng.uniform(1.5, 2.5), 2),
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("YOUR ASSIGNMENT VALUES - Lab 6: Integration Project")
    print("=" * 60)
